...
主な機能:
    - JD CSV のパース (wide → long 形式)
    - main race_id (18 桁) ↔ ts race_id (16 桁) のマッピング
    - 年単位でのキャッシュ付きバッチ読み込み

JR (馬連) / JW (枠連) は将来の拡張余地として残してあり、本モジュールでは扱わない。
"""
...
_JD_META_COLS = [
    "race_id_ts",     # 16 桁レース ID
    "snapshot_idx",   # 区分 (1=初期, 数字が大きいほど直前)
    "time_code",      # 月日時分 (MMDDHHMM)
    "horse_N",        # 出走頭数
    "win_votes",      # 単勝票数 (レース全体)
    "show_votes",     # 複勝票数 (レース全体)
]

# JRA は最大 18 頭まで対応 (実ファイルは 16 頭が主)
MAX_HORSES = 18


def race_id_main_to_ts(main_race_id: int | str) -> int:
    """main 形式 (18 桁: race + horse_num) → ts 形式 (16 桁: race のみ) に変換。

    main_race_id の下 2 桁が horse_num のため、先頭 16 桁を抽出する。
    """
...
def parse_jd_csv(path: str | Path, encoding: str = "shift_jis") -> pd.DataFrame:
    """単一の JD CSV を読み込み、long 形式の DataFrame を返す。

    Returns:
        DataFrame with columns:
            race_id_ts (int64), snapshot_idx (int), time_code (int),
            horse_N (int), win_votes (int), show_votes (int),
            horse_num (int 1..N), win_odds (float),
            show_odds_lo (float), show_odds_hi (float)

        単勝オッズ 0 の馬 (出走無し/取消) は除外。
        スナップショットが取れない、壊れたファイルは空 DataFrame を返す。
    """
    try:
    ...
    # 単勝オッズが 0 or NaN の馬はそのレースに存在しない/取消 → 除外
    long_df = long_df[long_df["win_odds"].fillna(0) > 0].reset_index(drop=True)
    return long_df


def _filename_year(filename: str) -> int | None:
    """JD CSV ファイル名から年 (2桁→4桁) を抽出する。

    期待形式: JD[place 2][year 2][td 2][race 2].CSV (計 10 桁の番号部)
    Returns 年 (int) または None (パース失敗時)。
    """
...
def aggregate_jd_per_horse(long_df: pd.DataFrame) -> pd.DataFrame:
    """long 形式の JD スナップショット集合を「馬 × レース」単位の集約特徴量に圧縮する。

    各 race × horse_num に対し以下を計算:
        - ts_n_snapshots:      取得スナップショット数
        - ts_win_first:        最古スナップショットの単勝オッズ
        - ts_win_last:         最新スナップショットの単勝オッズ
        - ts_win_min/max/std:  単勝オッズの統計
        - ts_win_drop_pct:     (first - last) / first  (オッズ低下率: 正=人気上昇)
        - ts_win_late_drop_pct:後半25%の low比 (直前急変動)
        - ts_show_lo_last/ts_show_hi_last: 直前の複勝オッズ
        - ts_show_drop_pct:    複勝オッズ中値の変動率
        - ts_implied_prob_last: 1/ts_win_last (粗いインプライド確率, 後で正規化)
        - ts_implied_prob_first: 1/ts_win_first
        - ts_anomaly:          人気序列の変化幅 (序盤→終盤)
        - ts_win_votes_last:   レース全体の単勝総票数 (最新スナップショット)
        - ts_show_votes_last:  レース全体の複勝総票数
        - ts_time_to_post_min: 最新スナップショットの月日時分 (整数のまま; 解析側で活用)
    """
    if long_df.empty:
        return pd.DataFrame()
    ...
    late_drops = g["win_odds"].apply(_late_drop)
    feat["ts_win_late_drop_pct"] = late_drops

    # レース内の人気序列 (低オッズほど人気) の序盤→終盤での変動
    ...
    # レース単位インプライド確率を正規化 (overround 補正)
...
def load_jd_for_years(
    ...
    """指定年の JD CSV をバッチ読み込みする (キャッシュ対応)。

    Args:
        ts_odds_dir: JD CSV が並ぶディレクトリ。
        years: 対象年 (4 桁) のリスト。
        cache_dir: キャッシュディレクトリ。指定時、年単位で parquet 保存・再利用。
        encoding: CSV エンコーディング。
        verbose: True で進捗ログ。
        aggregate: True (デフォルト) のとき、ファイル単位で集約特徴量に圧縮した上で結合する。
            False のときは生スナップショット (long 形式) を返す。

    Returns:
        集約特徴量 DataFrame (aggregate=True) または long 形式 DataFrame (aggregate=False)。
    """
    ...
    for year in sorted(set(years)):
        ...
        if verbose:
            print(f"  Year {year}: kept {len(year_frames)} files ({n_empty} empty/invalid)")
